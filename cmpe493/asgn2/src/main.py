...
article_pattern = re.compile(r'<reuters.*?</reuters>', flags=re.DOTALL)
topic_pattern = re.compile(pattern=r'<d>(.*?)</d>')
type_id_pattern = re.compile(pattern=r'lewissplit=\"(.*?)\".*newid=\"(.*?)\"')
...
def parse_data(text):
	...
	for article_text in articles_text:
		...
		article.id = match.group(2)
		# article.tpye = re.search(pattern=id_pattern, string=article_text).group(1)

		if not (article.type == "train" or article.type == "test"):
			continue

		_topics = re.findall(pattern=topic_pattern, string=article_text)

		# include only one topic from TOPICS
		intersect = list(set(_topics).intersection(TOPICS.keys()))
		if len(intersect) != 1:
			continue
		article.topic = intersect[0]

		title, body = "", ""
		try:
			title = re.search(pattern=title_pattern, string=article_text).group(1)
		except (IndexError, AttributeError):
			pass

		try:
			body = re.search(pattern=body_pattern, string=article_text).group(1)
		except (IndexError, AttributeError):
			pass
		...
		_words = tokenizer.stopwordsRemove(_words)  # remove stopwords from tokens
		_words = tokenizer.stemmize(_words)  # stemmize all words
...
def get_ready():
	...
	if os.path.exists(file_path):
		...
		for i in range(num_of_files):
			filename = "reut2-" + '{0:03d}'.format(i) + ".sgm"
			allTexts.append(read_file(filename))
